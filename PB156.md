...
**relace = dialog** -- spojení meti dvěma koncovými účastníky na úrovni bezprostředně vyšší, než je vrstva transportní (analogie telefonního hovoru)
...
* aplikační protokoly definují:
    ...
    * syntaxi zpráv
    * sémentiku zpráv
    * praviadla kdy a jak aplikace zprávy vysílají
...
* jednotměrný datový tok, definované požadavky na propusnost
...
* `method` -- protokol, který má být využit pro přístu ke zdroji
* `host` -- uzel, kde se nachází informace
* `port` -- volitelné, využívá se, pokud je použit jiný než strandardní port
...
* proces doručení elektronické zprávy:
    * doručení lokálnímu poštovnímu serveru -- ustanovení TCP spojení (porst 25) s poštovním severem
    * předání emailu cílovému poštovnímu serveru -- SMTP protkol
...
* bezpečná síť by měla nabízet služby:
    ...
    * Accounting (účtování)
* zabezpečená komuniace má důvernost, integritu a nepopiratelnost
...
**autentizace**  -- řezení přístupu, oprávnění nebo odepření přístupu
...
**integrita** -- ochrana dat před neutorizovanou modifikací
...
* jeden klíč
* výhody: nízká výpočetní náročnost, vhodné pro dlouhé zpárvy
...
* hashovací fuknce musí být jednosměrná (z hashe nelze získat zprávu) a one-to-one (z různých zpráv nedostaneme stejný hash - je to málo pravděpodobné)
* např. SHA-256

### Protokoly
* bezpečnostní koncepty lze realizovat na aplikačníí, transportní a síťové vrstvě
...
### Bezpčená architektura sítě
* základní principy a protkoly
...
* překryvová síť slouží k indxování a zjišťování peerů, po fyzické síti se přenáší data
...
* nevýhody: single point of failure, slabá škálovatelnost a robusnost, nízký výkon pro velký počet účastníků
...
* výhody: decentralizace, škálovatelnost, robusnost, výkon
* např. Gnutella
...
* Interface Massege Protocol (IMP) -- rozhraní mezi pc a sítí ARPANET
...
### Internet
* 1975 -- rozdělení ARPANETu na na ARPANET (adamecká půda) a MILNET (armáda)

...
* **staticky typovaný** -- typ (většiny proměnných znám v době překladu
...
* překlad je nutno provést zvlášť pro každou platformu
...
* respektovat již existující konvence v projektu
...
* návratová hodnota nebo předání ukazatele v argumentu
...
### Const
* konstantní hodnoty, které se nemění
...
* `void free(void*)`
* `void* memset(void * ptr, int value, size_t num)` -- nastaví paměť na zadanou hodnotu
...
* `union` zřídka
* `typedef` slouží k vytváření abstrakce a zkratek konkrétních datových typů (např. pole 10×10)
